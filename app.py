...
edu_fac = gpd.read_file("data/edu_facilities/hotosm_mys_education_facilities_polygons.shp")
financial = gpd.read_file("data/financial_service/hotosm_mys_financial_services_polygons.shp")
health = gpd.read_file("data/health_facilities/hotosm_mys_health_facilities_polygons.shp")
point_of_interest = gpd.read_file("data/point_of_interest/hotosm_mys_points_of_interest_polygons.shp")
...
point_of_interest = gpd.GeoDataFrame(point_of_interest, geometry=point_of_interest.geometry, crs="EPSG:4326")
...
point_of_interest = point_of_interest[['tourism', 'name', 'amenity', 'shop', 'geometry']]
...
if submit:
    ...
    with st.spinner():
        with col1:
            ...
            m.add_gdf(point_of_interest, layer_name="Point of Interest", style=style_extras)
        ...
        with col2:
            ...
            for index in range(len(airports)):
                if intersected_df['geometry'][0].contains(airports['geometry'][index]) == True:
                    airport_count.append(airports['name'][index])
            for index in range(len(edu_fac)):
                if intersected_df['geometry'][0].contains(edu_fac['geometry'][index]) == True:
                    edu_fac_count.append(edu_fac['name'][index])
            for index in range(len(financial)):
                if intersected_df['geometry'][0].contains(financial['geometry'][index]) == True:
                    financial_count.append(financial['name'][index])
            for index in range(len(health)):
                if intersected_df['geometry'][0].contains(health['geometry'][index]) == True:
                    health_count.append(health['name'][index])
            for index in range(len(seaports)):
                if intersected_df['geometry'][0].contains(seaports['geometry'][index]) == True:
            ...
            for index in range(len(point_of_interest)):
                if intersected_df['geometry'][0].contains(point_of_interest['geometry'][index]) == True:
            ...
            no_hotel = len(poi[(poi['Tourism'] == 'hotel') | (poi['Tourism'] == 'hostel')])
            no_theme_park = len(poi[(poi['Tourism'] == 'theme_park')])
            no_food_place = len(poi[(poi['Amenity'] == 'restaurant') | (poi['Amenity'] == 'cafe') | (poi['Amenity'] == 'fast_food')])
            no_fuel = len(poi[(poi['Amenity'] == 'fuel')])
            no_marketplace = len(poi[poi['Shop'] == 'marketplace'])
            no_supermarket = len(poi[(poi['Shop'] == 'supermarket') | (poi['Shop'] == 'convenience')])
            no_cloth_jewelry = len(poi[(poi['Shop'] == 'clothes') | (poi['Shop'] == 'jewelry')])
            no_mall = len(poi[(poi['Shop'] == 'mall') | (poi['Shop'] == 'department_store')])
            no_repair = len(poi[(poi['Shop'] == 'car_repair') | poi['Shop'] == 'motorcycle'])
            no_hair = len(poi[poi['Shop'] == 'hairdresser'])
            no_telco = len(poi[(poi['Shop'] == 'telecommunication')| (poi['Shop'] == 'electronics')])
            
            # make a dataframe
            poi_df = pd.DataFrame({
                'Place': ['Airport', 'Education Facilities', 'Financial Facilities', 'Health Facilities', 'Seaports', 'Hotel', 'Theme Park', 'Food Place', 'Fuel Station', 'Marketplace', 'Supermarket', 'Cloth & Jewelry', 'Mall', 'Repair Shops', 'Hair Salon', 'Telco'],
                'Count': [len(airport_count), len(edu_fac_count), len(financial_count), len(health_count), len(seaports), no_hotel, no_theme_park, no_food_place, no_fuel, no_marketplace, no_supermarket, no_cloth_jewelry, no_mall, no_repair, no_hair, no_telco]
            })
